...
public class KafkaContext implements ApplicationContextAware {
    ...
    public void getListener() {
        // 获取监听类
        Map<String, Object> beans = context.getBeansWithAnnotation(DmqListener.class);
        beans.forEach((key, bean) -> {
            Method[] methods = bean.getClass().getDeclaredMethods();
            for (Method method : methods) {
                if (method.isAnnotationPresent(DmqMapping.class)) {
                    DmqMapping annotation = method.getAnnotation(DmqMapping.class);
    ...
    public void handleReceive(Method method, Object bean, ConsumerRecord<String, String> record, Class<?> aClass) {
        try {
            ...
            receiveLogger.error("receive error:{}", e.getMessage());
            e.printStackTrace();
        }
    }
}
